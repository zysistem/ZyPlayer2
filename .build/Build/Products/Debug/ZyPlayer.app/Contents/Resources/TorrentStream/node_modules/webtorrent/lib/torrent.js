...
export default class Torrent extends EventEmitter {
  ...
  constructor (torrentId, client, opts) {
    ...
    // ZyPlayer patch: see the `assumeEmpty` branch in `_onMetadata`.
    this.assumeEmpty = !!opts.assumeEmpty
  ...
  get downloaded () {
    ...
    for (let index = 0, len = this.pieces.length; index < len; ++index) {
      if (this.bitfield.get(index)) { // verified data
        ...
        // ZyPlayer patch: a piece verified between the bitfield read and this
        // line is null. Unpatched, every read of `downloaded`/`progress` throws
        // and the helper's status reporting stops dead.
  ...
  async _onMetadata (metadata) {
    ...
    if (this.skipVerify) {
      ...
      this._onStore()
    } else if (this.assumeEmpty) {
      // ZyPlayer patch: the store was created empty a moment ago, so there is
      // nothing on disk to verify — the bitfield is already all zeros and the
      // pieces are freshly built. Hashing anyway is not just wasted work on a
      // multi-gigabyte sparse file: `_verifyPiecesUsingHash` calls
      // `_markUnverified` as it walks, which replaces the `Piece` objects the
      // download is filling in and clears their bits. Racing the live transfer,
      // it makes the buffered-piece count climb and fall for ever and playback
      // never starts. `skipVerify` is not the answer — that claims the opposite,
  ...
  _onWireWithMetadata (wire) {
    ...
    // ZyPlayer patch: send a *copy*. Handing the live BitField to the wire lets
    // the peer connection reuse its backing buffer, and bits that were set for
    // verified pieces come back cleared — measured as the buffered-piece count
    // falling as more peers connect, with no call to `bitfield.set(i, false)`
    // anywhere. Peers only need a snapshot of what we had at handshake time.
    else wire.bitfield(new BitField(this.bitfield.buffer.slice()))
  ...
  _updateWire (wire) {
    ...
    function speedRanker () {
      ...
      return index => {
        ...
        // ZyPlayer patch: same race as in `_request` — the piece can be gone.
  ...
  _hotswap (wire, index) {
    ...
    for (i = 0; i < minWire.requests.length; i++) {
      ...
      // ZyPlayer patch: same race as in `_request`.
  ...
  _request (wire, index, hotswap) {
    ...
    // ZyPlayer patch (webtorrent 3.0.16): a piece that completed while this
    // tick was queued is nulled out before its bit is set in the bitfield, so
    // the guard above lets it through and this line throws. An uncaught throw
    // here kills the whole helper process mid-playback.
